...
data {
  ...
  int nCovs;
  matrix[Tlen, nCovs] covs;
  int Nsharks;
  array[Tlen] int shark_index;
}
...
parameters {
  ...
  array[Nstates*(Nstates-1)] vector[Nsharks] randeff_tpm_raw;
  array[Nstates*(Nstates-1)] real mu_tpm;
  array[Nstates*(Nstates-1)] real<lower=0> sigma_tpm;
}
...
transformed parameters{
  ...
  array[Nstates*(Nstates-1)] vector[Nsharks] randeff_tpm;
  for (n in 1:(Nstates*(Nstates-1))) {
    randeff_tpm[n] = mu_tpm[n] + sigma_tpm[n]*randeff_tpm_raw[n];
  }
}
...
model {
  ...
  for (n in 1:(Nstates*(Nstates-1))) {
    randeff_tpm_raw[n] ~ std_normal();
    mu_tpm[n] ~ normal(-2, 1);
    sigma_tpm[n] ~ normal(0.2, 0.5);
  }
    ...
    for (i in 1:Nstates) {
      for (j in 1:Nstates) {
        if(i == j) {
          ...
          tpm[t,i,j] = exp(randeff_tpm[betarow, shark_index[t]]+ 
                            beta[betarow]*to_vector(covs[t]));
          betarow = betarow + 1;
        }
      }
    }
