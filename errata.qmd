...
- p. 34, Several Chapter 3 exercises were copies of Chapter 2 exercises. See [the online PDF](https://users.aalto.fi/~ave/Bayesian-Workflow.pdf) for the correct exercises.
...
- p. 63, "The likelihood is the only part of the data model that is needed for Bayesian inference conditional" $\rightarrow$ "The likelihood is the only part of the data model that is needed for Bayesian inference"
- p. 77, "$p|y\sim\mathrm{beta}(y+1, n-y+1)$" $\rightarrow$ "$\theta|y\sim\mathrm{beta}(y+1, n-y+1)$"
- p. 83, "$\tau=\sigma\sqrt{\nu_{\nu-2}}$" $\rightarrow$ "$\tau=\sigma\sqrt{\nu/(\nu-2)}$"
- p. 86, "something more informative such as $\exponential(0.1)$ or $\normal^+(0,10)$" $\rightarrow$ "something more informative such as $\exponential(1)$ or $\normal^+(0,1)$" (thanks to Marco Prevedello)
...
- p. 98, in Exercise 5.2c, "the conditional distributions $x|y\sim\mathrm{normal}(0.5y,1),\ x|y\sim\mathrm{normal}(0.5y,1)$" $\rightarrow$ "$x|y\sim\mathrm{normal}(0.5y,1),\ y|x\sim\mathrm{normal}(0.5x,1)$" (as printed, the two conditionals are identical and the exercise becomes trivial).
...
- p. 264, "between 1 and 5" $\rightarrow$ "between 0 and 5"
...
- p. 269, "the variance parameters are accurately estimated" $\rightarrow$ "the scale parameters are accurately estimated"
...
- p. 281, the priors on $\tau_0$ and $\sigma$ for the varying-intercept model as shown in the code as `prior3` are now the same as the priors discussed in the text
...
- p. 283, the priors on $\tau_0$, $\tau_1$, and $\sigma$ for the varying-intercept, varying-slope model as shown in the code as `prior4` are now the same as the priors discussed in the text
...
- p. 311, "the point estimate from Figure 19.3a of $\mu_{\delta}$, which is 1.58 ... $\logit^{-1}(1.58 \pm 0.3)$, which is $(0.78, 0.87)$" $\rightarrow$ "which is 1.54 ... $\logit^{-1}(1.54 \pm 0.3)$, which is $(0.78, 0.86)$" (matching the posterior median of $\mu_{\delta}$ in the table on p. 310).
- p. 312, "We do not believe these tests have specificities below 50\%" $\rightarrow$ "We do not believe these tests have sensitivities below 50\%".
- p. 313, "roughly consistent with inter-site variation in specificity" $\rightarrow$ "roughly consistent with inter-site variation in sensitivity".
...
- p. 326, in Figure 21.4 the in-figure row subtitles for the third and fourth rows of posterior predictive simulations are both "hier logit model". The third row is the two-parameter logarithmic model (M2) and the fourth row is the hierarchical two-parameter logarithmic model (M4).
...
- p. 329, in the sequential posterior predictive simulation code, `dogs_df_pred$prev_pred_shock <- as.numeric(rowSums(pred_shock))` $\rightarrow$ `dogs_df_pred$prev_shock <- as.numeric(rowSums(pred_shock))`. 
- pp. 330--331, the Stan program shown is the version without priors. The Stan 
program should instead have the `parameters` block
```stan
parameters {
...
```
and, in place of the `logit_ab[j,] ~ multi_normal(...)` line in the `model` block, the priors from the display on p. 329:
```stan
...
```
The complete program is `dogs_5.stan` in the `dogs` case study directory and has
been fixed in the book PDF.

- pp. 334--337, four references to "model 5" $\rightarrow$ "model 4"
- p. 335, in Figure 21.12 both in-figure panel titles read "Model 4: hier. logit". 
These should read "hier. log".
- p. 336, the priors in the Stan program have been changed to match the model displayed on p. 329 (see the pp. 330--331 entry above). With those priors the posterior median of the correlation between $\mathrm{logit}(a)_j$ and $\mathrm{logit}(b)_j$ is $-0.2$ with a posterior standard deviation of $0.5$, rather than $-0.1$ and $0.4$ as printed.
