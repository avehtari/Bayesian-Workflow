...
# Declining exponentials and sums of declining exponentials 
...
distributions centered at 0 with standard deviation 10.  In addition,
...
between 0 and 10, from the above model with $a=0.2$, $b=0.3$, $\sigma=0.5$.
...
mod_1 <- cmdstan_model("exponential.stan")
...
```{r, results=FALSE}
mod_1b <- cmdstan_model("exponential_positive.stan")
...
mod_3 <- cmdstan_model("exponential_positive_lognormal.stan")
...
concentration of a drug in the blood of someone who was given a
specified dose at time 0.  In a simple _two-compartment model_, the
...
$b$ to be the second.  We added the `positive_ordered` type to
...
mod_4 <- cmdstan_model("sum_of_exponentials.stan")
...
The two lines are strikingly close, and it would be essentially impossible to
tell them apart based on noisy data, even 1000 measurements. So Stan had trouble
recovering the true parameters from the data.
...
problem in this case is that, since only one term in the model was
...
in the prior or likelihood that is keeping them from doing so.

To fix the problem, we can add some prior information.  Here we shall
use our default, which is independent $\mbox{normal}(0,1)$ prior
...
point we shall assign default priors to everything.  The priors are in
...
```{r, results=FALSE}
mod_5_with_priors <- cmdstan_model("sum_of_exponentials_with_priors.stan")
...
we'd probably expect the parameters $b_1$ and $b_2$ to be roughly on
...
values of $y$ ranged in the millions, then the coefficients $a$ would
have to be of order $10^6$, and, again, we would want to rescale the
...
No matter how it's done, inference has to come from somewhere, and if
