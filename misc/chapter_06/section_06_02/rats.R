...
plot(range(x), c(0,1), xlab="dose (log g/ml)", ylab="Pr (death)", bty="l", type="n")
for (s in sample(nrow(sims_1), 20)) {
  curve(invlogit(sims_1$a[s] + sims_1$b[s] * x), from=-1, to=1, col="red", lwd=0.5, add=TRUE)
}
curve(invlogit(mle_1["a"]+ mle_1["b"]* x), from=-1, to=1, col="blue", lwd=2, add=TRUE)
points(x, y/n, pch=20, cex=2)
dev.off()


fit_2 <- logistic_2$sample(data=c(rats_data, mu_a=0, mu_b=0, sigma_a=5, sigma_b=5), refresh=0)
print(fit_2)
...
for (s in sample(nrow(sims_2), 20)) {
  curve(invlogit(sims_2$a[s] + sims_2$b[s] * x), from=-1, to=1, col="red", lwd=0.5, add=TRUE)
}
a_post_mean <- mean(sims_2$a)
b_post_mean <- mean(sims_2$b)
curve(invlogit(a_post_mean + b_post_mean * x), from=-1, to=1, col="blue", lwd=2, add=TRUE)
points(x, y/n, pch=20, cex=2)
dev.off()

fit_3 <- logistic_3$sample(data=c(rats_data, mu_a=0, mu_b=0, sigma_a=5, sigma_b=5), refresh=0)
print(fit_3)
...
plot(range(x), c(0,1), xlab="dose (log g/ml)", ylab="Pr (death)", bty="l", type="n")
for (s in sample(nrow(sims_2), 20)) {
  curve(invlogit(sims_3$a[s] + sims_3$b[s] * x), from=-1, to=1, col="red", lwd=0.5, add=TRUE)
}
...
plot(sims_3$a, sims_3$b, xlab="a", ylab="b", xaxt="n", yaxt="n", pch=20, col="red", cex=.1)
axis(1, seq(-2,4,2))
axis(2, seq(0,20,10))
...
plot(range(x), c(0,1), xlab="dose (log g/ml)", ylab="Pr (death)", bty="l", type="n")
b_post_mean <- mean(sims_3$b)
curve(invlogit(a_post_mean + b_post_mean * x), from=-1, to=1, col="blue", add=TRUE)
lines(x_grid, Ey_grid, col="red")
points(x, y/n, pch=20, cex=2)
text(0.1, 0.85, expression(paste("logit"^{-1}, (hat(a) + hat(b)*x))), adj=1, cex=.9, col="blue")
