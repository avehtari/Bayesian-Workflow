...
plot(x, y, pch=20, cex=.5, xlab="x", ylab="y", bty="l", main="Data and posterior draws of fitted regression line", cex.main=.9)
for (k in sample(ndraws(sims), 50)) {
  curve(extract_variable(sims, "a")[k] + extract_variable(sims, "b")[k] * x, add=TRUE, lwd=.5, col="blue")
}
...
plot(range(x_tilde), range(intervals), xlab=expression(tilde(x)), ylab=expression(paste("Simulated 80% predictive intervals for ", tilde(y))), bty="l", type="n",
