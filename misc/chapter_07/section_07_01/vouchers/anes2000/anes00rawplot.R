...
dat <- read.dta("naes00new.dta")
favvoucher <- dat$favvoucher
inc <- dat$inc
eth <- dat$ethn
st <- dat$st
...
nn <- array (NA, c(n.state,n.inc), dimnames=namelist2)
design.effect.by.cell <- array (NA, c(n.state,n.inc,n.releth), dimnames=namelist3)
design.effect.by.cell.2way <- array (NA, c(n.state,n.inc), dimnames=namelist2)
for (i in (1:n.state)){
  if (!(state.abb.long[i] %in% c("AK","DC","HI"))){
    for (j in 1:n.inc){
        okk <- state==i & inc==j
        ybar.weighted.2way[i,j] <- wmean(y, pop.weight, okk)
        nn[i,j] <- sum(okk)
        design.effect.by.cell.2way[i,j] <- 1 + var(pop.weight[okk]/mean(pop.weight[okk]))
      for (k in 1:n.releth){
...
ybar.weighted.2way[is.nan(ybar.weighted.2way)] <- NA
design.effect2 <- wmean (design.effect.by.cell.2way, nn, nn>1)
nn.eff <- nn/design.effect2
...
# Make the map  
#png (paste ("vouchermapsraw", 2000, ".png", sep=""), height=1000, width=1000)
pdf (paste ("vouchermapsraw", 2000, ".pdf", sep=""), height=14.5, width=14.5)
#====================
v.idx <- seq(1, 8)
h.idx <- c(0, seq(9, 13))
...
#=====================
national.avg <- mean(y, na.rm=TRUE)
for (k in 0:7){
...
for (j in 1:n.inc){
  for (k in 0:7){
    if (k==0){
      statemaps (colorblind (-(ybar.weighted.2way[,j]-national.avg), lo=-.25, hi=.25))
    }
    else {
      statemaps (ifelse (prop.voters[,j,k]>.01, colorblind (-(ybar.weighted[,j,k] - national.avg), lo=-.25, hi=.25), "white"))
    }
  }
}
par(mar=c(2,0,1,0))
